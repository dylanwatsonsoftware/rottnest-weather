...
const FACILITY_NAMES = new Set(facilities.map((place) => place.name));
...
test('place media covers every known landmark and food venue with local image files', () => {
    ...
    for (const [placeName, images] of Object.entries(PLACE_MEDIA)) {
        assert.ok(
            LANDMARK_NAMES.has(placeName) || FACILITY_NAMES.has(placeName),
            `${placeName} should exist as a landmark or bundled facility`
        );
